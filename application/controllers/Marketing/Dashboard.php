    ...
    function Simpaninteraksi() {
        ...
        if ($cekfoto == 0) {
            ...
            $data = array(
                ...
                'KOTA_KAB' => $this->input->post('KOTA_KAB'), 'PROVINSI' => $this->input->post('PROVINSI'), 'KODEPOS' => $this->input->post('KODEPOS'), 'TELEPON' => $this->input->post('TELEPON'), 'KODE_INTERAKSIKUNJ' => $this->input->post('KODE_INTERAKSIKUNJ'), 'KETERANGAN' => $this->input->post('KETERANGAN'), 'TGL_KUNJ' => date("Y-m-d"), 'JAM_KUNJ' => date("h:i:s"), 'ALAMAT_VALID' => $this->input->post('ALAMAT_VALID'), 'BERTEMU_PENS' => $this->input->post('BERTEMU_PENS'), 'MANFAATPENS_BTPN' => $this->input->post('MANFAATPENS_BTPN'), 'HP_STATUS' => $this->input->post('HP_STATUS'), 'HP_NOMINAL' => $this->input->post('HP_NOMINAL'), 'ttd' => $this->input->post('ttd', FALSE), 'status' => 2);
